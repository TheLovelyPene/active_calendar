# Script to add August-December 2025 events to the main database
# This script will insert the new events before the closing bracket of the events list

# New events to add (August-December 2025)
new_events = [
    # --- NYC Events ---
    {
...
def format_event_for_database(event):
    """Format an event dictionary as a string for insertion into the database"""
    return f'''    {{
        "name": "{event["name"]}",
        "date": "{event["date"]}",
        "time": "{event["time"]}",
...
def add_events_to_database():
    """Add the new events to the database file"""
    database_file = "nyc_free_events_database_2.py"
    
    # Read the current database file
    with open(database_file, 'r') as f:
        content = f.read()
    
    # Find the position to insert new events (before the closing bracket)
    insert_position = content.find('];')
    
    if insert_position == -1:
        print("Error: Could not find the closing bracket of the events list")
        return
    
    # Format all new events
    new_events_text = ""
    for event in new_events:
        new_events_text += format_event_for_database(event) + ",\n"
    
    # Remove the trailing comma from the last event
    new_events_text = new_events_text.rstrip(",\n") + "\n"
    
    # Insert the new events
    updated_content = content[:insert_position] + new_events_text + content[insert_position:]
    
    # Write the updated content back to the file
    with open(database_file, 'w') as f:
        f.write(updated_content)
    
    print(f"Successfully added {len(new_events)} new events to {database_file}")
